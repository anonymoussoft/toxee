...
# Scenarios: S26
# Class: 2proc-l3
# Run the S26 "accept a friend request creates a friendship" L3 gate.
#
...
# to reach A, has A accept it, and asserts B is now in A's friends[] with the
# pending application gone from A's friendApplications[].
set -euo pipefail
...
echo "[accept] launching FRESH pair (no fixture restore)"
"$MCP_DIR/launch_fixture_c_pair.sh"

a_ws="$(jq -r '.instances.A.ws_uri' "$PAIR_JSON")"
b_ws="$(jq -r '.instances.B.ws_uri' "$PAIR_JSON")"

dart run tool/mcp_test/drive_fixture_c_accept.dart "$a_ws" "$b_ws"

echo "[accept] S26 PASS"
