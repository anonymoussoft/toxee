...
**Status**: spec-only (L1 WidgetTester gate owed for the callback-fires leg; the image-clipboard write + QR render stay L3). No marionette-driven runnable gate. **There is NO data-half `l3_*` gate for the QR copy** (unlike S102's `l3_self_id.json`), because the copied artifact is the QR PNG, not a `l3_dump_state` field.
