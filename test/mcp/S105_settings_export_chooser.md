...
**Status**: spec-only — the chooser surface is keyed/assertable (L1 gate owed, recipe above) but has no marionette-driven runnable gate; the file-write leg cross-refs S43 (which has its own `l3_set_export_save_path` seam, NOT a runner gate).
