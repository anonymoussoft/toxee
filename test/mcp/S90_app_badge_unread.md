...
**Status**: driveable — the inbound source (echo peer or paired B) + `l3_dump_state.totalUnreadCount` (top-level, `l3_debug_tools.dart:2922`) + `l3_mark_read` (`l3_debug_tools.dart:1010-1090`) already exist; the count round-trip (0 → N → 0) is fully runner-assertable. No executable `run_*.sh` ships for THIS scenario yet — the assertions below are the gate contract a `drive_fixture_c_badge.dart` (echo or paired) would encode. The OS dock/taskbar badge PIXEL is out of scope (OS-only); the COUNT is the assertion.
