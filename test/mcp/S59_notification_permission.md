...
**Status**: the in-app permission CONSEQUENCE is now gated (2026-06-08); only the OS GRANT itself is irreducible. The macOS/iOS authorization (TCC) is decided by the OS at `NotificationService.init()` and cannot be granted/denied programmatically — that grant is the irreducible OS gate. But the product BEHAVIOR it drives — a DENIED permission must SUPPRESS the banner, a granted one must let it through — is now L1-tested: `test/notifications/notification_permission_gate_test.dart` mocks the `dexterous.com/flutter/local_notifications` channel and, via two `@visibleForTesting` seams (`debugForceIsAndroid` to take the Android gate path on the mac host + `debugAndroidPermissionGranted` to seed the cached result, bypassing the host-null plugin), asserts `showMessageNotification` emits NO platform `show` when denied and DOES when granted (notification_service.dart gate ~328). So the deny→no-banner / allow→banner consequence is deterministically gated; the actual OS grant decision stays out of scope. (Notification-tap routing + mute-suppression are covered by S53/S83.) Shared desktop+mobile (the gate is platform-agnostic given the seam).
**Covered-by**: `test/notifications/notification_permission_gate_test.dart`
