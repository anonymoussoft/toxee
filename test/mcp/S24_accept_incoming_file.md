...
**Status**: covered by executable Fixture C gate — `tool/mcp_test/run_fixture_c_file.sh` (B AUTO-accepts the small inbound file under its size limit; asserts a non-self file message arrives with a written non-empty `filePath`). Validated live 2026-06-01. Explicit large-file/manual accept (a dedicated `l3_accept_file` over `acceptFileTransfer`) remains a follow-up.
