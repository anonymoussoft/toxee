...
**Status**: not applicable — the product has NO call-duration/inactivity timeout that auto-ends an established call (verified: callDuration increments for display only; no max-duration check exists). The two real call timeouts are covered elsewhere: the unanswered-ring path is S77 (`run_fixture_c_missed_call.sh`) and the network-drop 8s reconnect grace is S69 (`run_fixture_c_network_drop.sh`).
