...
**Status**: covered (executable, two-process) for the accept DATA-path via `tool/mcp_test/run_fixture_c_accept.sh` (S26's gate). The DETAIL-screen UI-control angle (open the row → accept on the pushed detail page) is **spec-only** — the runnable gate accepts via `l3_accept_friend_request`, not via the detail-screen button. Live-validation of the detail-screen tap owed (two-process).
