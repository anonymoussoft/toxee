...
**Status**: NOT runner-gated for the data half. `l3_clear_history` is **C2C-only** and explicitly rejects group ids (`l3_debug_tools.dart:1251-1255`, error `group_unsupported`; coverage map line 650-651). The ONLY executable path today is the marionette UI tap (verify-via-dump-state below). A future `l3_set_group_recv_opt` / `l3_clear_group_history` tool is the gate.
