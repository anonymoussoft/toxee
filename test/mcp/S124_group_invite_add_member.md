...
**Status**: covered (invite-send + delivery FIXED in Dart). The keyed add-member UI walk is also covered at the widget layer (L1): for a regular NGC group (`GroupType.Work`) the real picker is driven — tap the keyed friend rows `add_member_contact_item:<userID>`, tap `group_member_invite_confirm_button` — and the captured production `inviteUserToGroup` carries the exact target group id and exactly the selected member ids (the friend-PICKER per-row key gap is now RESOLVED). Gate `run_fixture_c_group_invite.sh`. Two-process NGC reliability is the native residual (S81). Shared with S81/S47.
**Covered-by**: `test/ui/group/group_add_member_real_ui_test.dart` (S124 widget gate, confirm-parameter fidelity: group id + selected member ids)
...
- **Picker key gap — RESOLVED**: the add-member picker rows are now keyed `add_member_contact_item:<userID>` (`tencent_cloud_chat_group_add_member.dart:351`) in addition to the ENTRY (`group_add_member_button`, body:1586) and the CONFIRM (`group_member_invite_confirm_button`, add_member:90). The widget-layer gate selects rows by their per-row key; only the two-process flutter_skill leg ever needed the text/semantic fallback.
